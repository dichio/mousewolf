...
# if on remote
os.chdir("/home/vdichio/projects/mousewolf")
...
def random_init(K, rng=None):
    ...
    # Use alpha > 1 to encourage staying in the same state
...
def fit_one(mouse, week, K, sequences, n_trials):
    ...
    K         : int   — number of hidden states
    sequences : list  — list of (stim, lick) tuples
    ...
    best_params, best_log_lik, all_traces = fit_hmm(
        sequences, K=K, n_restarts=10, verbose=False
    )

    print(f"  Done {mouse} | week {week} | K={K} | log-lik: {best_log_lik:.4f}")
...
if __name__ == "__main__":
    ...
    # ── Build all jobs ─────────────────────────────────────────────────────────
    jobs = []  # list of (mouse, week, K, sequences, n_trials)
    ...
    # n_jobs=4  uses 4 CPUs (recommended on shared servers!)
    outputs = Parallel(n_jobs=4, verbose=10)(
    ...
    # Save results
    with open("data/hmm_results.pkl", "wb") as f:
        pickle.dump(results, f)

    print("\nSaved: data/hmm_results.pkl")
